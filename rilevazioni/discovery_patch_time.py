...
num_switch = 3 #da modificare per le rilevazioni
len_massima = (num_switch-1) * 2 #max numero rilevazioni
tempi_hmac = []
tempi_link = [] 

def scrivi_tempi():
    if len(tempi_link) == len_massima:
        #scrittura su file
        if( not os.path.exists("/home/rilevazioni.csv")):
            f = open("/home/rilevazioni_lldp/rilevazioni_PATCH.csv", "w")
            f.write("tempi_detection_link,numero_switch")
            f.write('\n')
        else: 
            f = open("/home/rilevazioni_lldp/rilevazioni_PATCH.csv", "a")
        for t in tempi_link:
            f.write(t)
            f.write(',')
            f.write(str(num_switch))
            f.write('\n')
        f.close()
        print("HO SCRITTO LINK SU FILE")
        if( not os.path.exists("/home/rilevazioni_lldp/rilevazioni_create_PATCH.csv")):
            f = open("/home/rilevazioni_lldp/rilevazioni_create_PATCH.csv", "w")
            f.write("tempi_creazione_packet_out")
        else: 
            f = open("/home/rilevazioni_lldp/rilevazioni_create_PATCH.csv", "a")
            f.write("tempi_creazione_packet_out")
        f.write('\n')   
        for t in tempi_hmac: 
            f.write(t)
...
class LLDPSender (object):
  ...
  def _create_discovery_packet (dpid, port_num, port_addr, ttl):
    ...
    #print("L'aggiunta di HMAC ha impegato: {}".format(total_time))
    tempi_hmac.append(str(total_time))
    return eth
        ...
        for line in t.payload.decode().split('\n'):
          if line.startswith('dpid:'):
            ...
            if smac[1][:64] != hmac_encryption(lldph.tlvs[1].id, lldph.tlvs[0].id, lldph.tlvs[2].ttl, _hmac_key):
              ...
              end_time = time.time()
              tempi_link.append(str(end_time - start_time))
              scrivi_tempi()
              return EventHalt
