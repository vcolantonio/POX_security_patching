...
tempi_link = []
tempi_create_discovery = []

def scrivi_tempi():
    if len(tempi_link) == len_massima:
        #scrittura su file
        if( not os.path.exists("/home/rilevazioni.csv")):
            f = open("/home/rilevazioni_lldp/rilevazioni_ORIGINAL.csv", "w")
            f.write("tempi_detection_link,numero_switch")
            f.write('\n')
        else: 
            f = open("/home/rilevazioni_lldp/rilevazioni_ORIGINAL.csv", "a")
        for t in tempi_link:
            f.write(t)
            f.write(',')
        ...
        if( not os.path.exists("/home/rilevazioni_lldp/rilevazioni_HMAC.csv")):
            f = open("/home/rilevazioni_lldp/rilevazioni_create_ORIGINAL.csv", "w")
            f.write("tempi_creazione_packet_out")
        else: 
            f = open("/home/rilevazioni_lldp/rilevazioni_create_ORIGINAL.csv", "a")
            f.write("tempi_creazione_packet_out")
        f.write('\n')   
        for t in tempi_create_discovery: 
            f.write(t)
            f.write('\n')
        f.close()
        print("HO SCRITTO CREATE SU FILE")
        #STOPPA IL PROGRAMMA
...
class LLDPSender (object):
  ...
  def _create_discovery_packet (dpid, port_num, port_addr, ttl):
    ...
    tempi_create_discovery.append(str(end - start))
    scrivi_tempi()
    return eth
